...
public class ImpressionConversion
{
        ...
        public static class ReduceClass extends Reducer<Text, Text, Text, Text>
        {
        	...
        	Text vword = new Text();
                ...
                if(conversions.size() > 0 && impressions.size() > 0)
                {
                	...
                	if(conSeconds > impSeconds)
                	{
                		long timediff = (conSeconds - impSeconds) / 1000;
                        	StringBuilder emitValue = new StringBuilder(1024);
                        	emitValue.append(latestImpTime).append("\t").append(latestConTime).append("\t").append(timediff);
                        	vword.set(emitValue.toString());
        ...
        public static void main(String args[]) throws Exception
        {
                ...
                {
                        System.out.println("Usage: ImpressionConversion <input> <output>");
                        System.exit(1);
                }
               	Job job = new Job(conf, "This MapReduce job for getting Impression to Conversion");
                job.setJarByClass(ImpressionConversion.class);
